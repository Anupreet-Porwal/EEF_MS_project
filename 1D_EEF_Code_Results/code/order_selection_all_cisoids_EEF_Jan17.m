...
% MODEL SELECTION OF CISOIDS WITH EEF 
% January 17, 2017
...
nsamparr=[5,15,25,50,100,200,400];
for z = 1:length(nsamparr);

numsamp=nsamparr(z);    % 'Sample Size',numsamp
sigsq=1;
...
if eef_rn> (3*comp+1)
    eef_obj(comp)=eef_rn-(3*comp+1)*(1+log(eef_rn/(3*comp+1)));
else
    eef_obj(comp)=0;
end
...
result_all=[bic;bicc;eef;pal;aic]
...
result_diff_n((count+1):(count+5),1)=numsamp;
result_diff_n((count+1):(count+5),2)=['B'; 'c'; 'E'; 'P';'A'];
result_diff_n((count+1):(count+5),3:12)=result_all;

% 
count=count+5;
end% loop for numsamp
xlswrite('result_numsamp_all_sigmasq_1.xlsx', result_diff_n,'Sheet1');