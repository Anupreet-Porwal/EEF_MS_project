...
nsamparr=[2,3,4,5,7,9,10,15,20,30,40];
...
if eef_rn> (4*comp+1)
    eef_obj(comp)=eef_rn-(4*comp+1)*(1+log(eef_rn/(4*comp+1)));
else
    eef_obj(comp)=0;
end
...
result_diff_n((count+1):(count+4),2)=['B'; 'P'; 'E'; 'A'];
result_diff_n((count+1):(count+4),3:7)=result_all;


count=count+4;
end% loop for numsamp
xlswrite('result_numsamp_all_sigmasq_15.xlsx', result_diff_n,'Sheet1');